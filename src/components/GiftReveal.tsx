...
export function GiftReveal() {
  ...
  const goTo = (index: number) => {
    ...
    // Programmatically scroll the slider
  ...
  return (
    <>
      ...
      <button
        onClick={handleOpen}
        disabled={isOpen}
        className="fixed bottom-6 right-6 z-50 group flex items-center justify-center w-16 h-16 bg-gradient-to-br from-amber-500 to-amber-600 rounded-full shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-70 disabled:cursor-wait"
        type="button"
      >
      ...
      {!isOpen && (
        <div className="fixed bottom-24 right-6 z-50 max-w-[140px] text-right pointer-events-none">
          <span
            className="inline-block text-xs font-medium text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 shadow-sm"
      ...
      {/* ═══ Gift box — everything happens inside here ═══ */}
      {isOpen && (
        <div
            ...
            {phase === 'countdown' && (
              <div className="flex flex-col items-center justify-center aspect-[3/4] px-6">
                <div className="relative flex flex-col items-center gap-4">
                  <div
                    key={`ring-${count}`}
                    className="absolute w-28 h-28 rounded-full border-2 border-amber-400/40 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
                    style={{ animation: 'giftCountdownRing 1s ease-out forwards' }}
                  />
                  <span
                    key={`num-${count}`}
                    className="text-[80px] font-bold text-amber-300 select-none"
                    style={{
                      fontFamily: "'Playfair Display', serif",
                      textShadow: '0 0 40px rgba(251,191,36,0.5), 0 0 80px rgba(217,119,6,0.3)',
                      animation: 'giftCountdownPop 0.8s cubic-bezier(0.34,1.56,0.64,1) forwards',
                    }}
                  >
                    {count}
                  </span>
                  <p className="text-amber-200/50 text-sm italic" style={{ fontFamily: "'Playfair Display', serif" }}>
            ...
            {phase === 'blast' && (
              <div className="relative flex flex-col items-center justify-center aspect-[3/4] px-6 overflow-hidden">
                ...
                {Array.from({ length: 30 }).map((_, i) => {
                  ...
                  const shades = ['#F59E0B', '#FBBF24', '#FCD34D', '#D97706'];
                  return (
                    <div
                      key={i}
                      className="absolute"
            ...
            {phase === 'reveal' && (
              <>
                ...
                <div className="relative px-6 pt-6 pb-4 text-center shrink-0">
                  <p className="text-amber-400 text-xs tracking-[0.2em] uppercase font-medium mb-1">A Gift For You</p>
                  <h3 className="text-amber-100 text-xl font-semibold" style={{ fontFamily: "'Playfair Display', serif", fontStyle: 'italic' }}>
                    Happy Birthday
                  </h3>
                  <p className="text-amber-200/50 text-sm mt-1" style={{ fontFamily: "'Playfair Display', serif" }}>
                  ...
                  >
                    {giftImages.map((img, i) => (
                      <div key={i} className="snap-center shrink-0 w-full h-full flex items-center justify-center px-4">
                        <div className="relative w-full h-full rounded-2xl overflow-hidden bg-amber-900/30">
                    ...
                    {giftImages.map((_, i) => (
                      <button
                        ...
                        className={`rounded-full transition-all duration-300 ${
                          i === currentSlide ? 'w-5 h-1.5 bg-amber-400' : 'w-1.5 h-1.5 bg-white/30 hover:bg-white/60'
                        }`}
                        type="button"
                      />
                    ))}
                ...
                <div className="flex items-center justify-center gap-2 py-3 shrink-0 border-t border-white/10">
                  <span className="material-symbols-outlined text-amber-400 text-base animate-pulse" style={{ fontVariationSettings: "'FILL' 1" }}>music_note</span>
                  <span className="text-xs text-amber-200/60 font-medium">Now playing: Elohim</span>
                </div>
              </>
            )}
          </div>
